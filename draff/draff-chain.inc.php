...
// and not directly by the stream and step objects
    ...
    public $chn_plugins = array();  // array of plugin objects of various types
...
// these items are passed to functions in the current script
    public $chn_app_globals; // object intended for entire set of apps - and a single definition for entire set of apps, not just one script
    public $chn_app_data;  // data local, and defined by script, has abstract methods
    public $chn_app_emitter = NULL;
...
//--- the current form key is stored in the "stream"
    ...
    private $chn_form_curKey = NULL;  // current step key - associated with stream
     ...
     function __construct( $emitterClassName) {
        ...
        $this->chn_url_rsmMode = isset($this->chn_url_params['drfMode']) ? $this->chn_url_params['drfMode'] : NULL;
    ...
    function chn_register_appGlobals($appGlobals) {
        $this->chn_app_globals = $appGlobals;
        return $appGlobals;
    }
    
    function chn_register_appData($appData) {
        $this->chn_app_data = $appData;
        return $appData;
    }
    
    function chn_register_emitter($emitter) {
        $this->chn_app_emitter = $emitter;
        return $emitter;
    }
...
//============================================================
    ...
    function chn_form_launch($formIndex = NULL) {
        ...
        if ($formIndex === NULL) {  // will always be true after a redirect ?????
            if (isset($this->chn_url_params['drfForm'])) { // argument takes priority
                // only use once - do not inherit as specified form must be able to launch another form
                $formIndex = $this->chn_url_params['drfForm'];
                unset($this->chn_url_params['drfForm']);
        ...
        //    $this->chn_app_emitter = new $this->chn_app_emitterName ($this->chn_app_globals, $this->chn_form_current, 'draff-html-select'); // ??????????? - eliminate style - put in content
            if (!empty($_POST)) {
                ...
                // $this->chn_app_emitter = new $this->chn_app_emitterName ($this->chn_app_globals,$this->chn_form_current,'draff-html-select'); // ??????????? - eliminate style - put in content
                $this->chn_state->ses_set('#curFormId', $this->chn_form_curKey);  //???? chain may not exist
                $this->chn_form_current->drForm_process_submit($this->chn_app_data, $this->chn_app_globals, $this, $this->chn_submit);
...
//        $this->chn_form_current->drForm_initHtml( $this->chn_app_data, $this->chn_app_globals, $this, $this->chn_app_emitter );
            $this->chn_form_current->drForm_process_output($this->chn_app_data, $this->chn_app_globals, $this, $this->chn_app_emitter);
            //?????????????? what happens now ???????
            exit;  // after everything has been outputted (to the web page) all done
        }
        ...
        $this->chn_url_redirect(array('drfForm' => $this->chn_form_curKey));
    }
...
//============================================================
    ...
    function chn_url_redirect(...$options) {
        $url = $this->chn_url_getString(...$options);
...
//============================================================
    ...
    function chn_register_plugin($pluginName, $pluginObject) {
        $this->chn_plugins[$pluginName] = $pluginObject;
    }
