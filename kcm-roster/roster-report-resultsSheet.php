...
//--- roster-report-resultsSheet.php ---
...
class form_kcm1_tallyReport extends Draff_Form {  // specify winner

function drForm_processSubmit ( $appData, $appGlobals, $appChain ) {
    $appData->apd_formData_get( $appGlobals, $appChain );
    $appData->reportFilters->rf_form_processExportSubmit( $appData, $appGlobals, $appChain, $submit );
}
...
function drForm_initData( $appData, $appGlobals, $appChain ) {
    ...
    $appData->apd_tallyReport = new kcr_report_nameLabels($appData->apd_roster_program->prog_programId,'h');
}
...
function drForm_initHtml( $appData, $appGlobals, $appChain, $appEmitter ) {
    ...
    kcmRosterLib_setBannerSubTitle($appEmitter,$appGlobals, $appData->apd_roster_program,'Results (Games and Points)');
...
class appData_resultsSheet extends draff_appData {
...
function apd_formData_get( $appGlobals, $appChain ) {
    ...
    //$this->apd_tallyReport = new kcr_report_nameLabels($this->apd_roster_program->rst_program,'h');
    $this->reportFilters = new report_filters('Roster');;
    $this->reportFilters->rf_export_pdf = TRUE;
    $this->reportFilters->rf_export_excel = FALSE;
...
$roster = new kcm_roster($this->kcr_programId);
...
$appData = new appData_resultsSheet;
$appChain = new Draff_Chain( $appData, $appGlobals, 'kcmKernel_emitter' );

$appData->apd_roster_program = new pPr_program_extended_forRoster($appGlobals);
$appData->apd_roster_program->rst_load_rosterData($appGlobals, $appChain);

$appChain->chn_form_register(1,'form_kcm1_tallyReport');
