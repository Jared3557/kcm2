...
Class appForm_staff_select extends Draff_Form {
...
Class appForm_staff_view extends Draff_Form {
...
Class appForm_staff_report extends Draff_Form {
...
function stdRpt_initStyles($appEmitter) {
   ...
   $appEmitter->addOption_styleTag('.co-name','width:9rem;');
   $appEmitter->addOption_styleTag('.co-phone','width:3rem');
   $appEmitter->addOption_styleTag('span.loc-short','margin-left:1.2em;');
   $appEmitter->addOption_styleTag('span.loc-email1','font-size:0.5em;');
   $appEmitter->addOption_styleTag('span.loc-email2','font-size:1.0em;;');
   $appEmitter->addOption_styleTag('a.loc-link-as-button','background-color: #cfc;display: inline-block;border: 1px solid #8b8;text-align: center;padding: 0.1em 0.5em 0.1em 0.5em;	border-radius: 0.6em; margin: 0.1em 0.1em 0.1em 0.1em;');
}
...
function stdRpt_output($appEmitter,$appGlobals) {
    ...
    $appEmitter->table_head_start();
    $appEmitter->krnEmit_reportTitleRow('Staff List Report',5);
    $appEmitter->row_start('rpt-grid-row');
...
function apd_getRecord_staff( $appGlobals, $appChain ) {
    ...
    if ($this->apd_staff_id == 0) {
        $this->apd_staff_rec = $this->apd_getRecord_office($appGlobals);
    }
    else {
...
private function apd_getRecord_office($appGlobals) {
    $row['sSt:StaffId'] = '0';
    $row['sSt:ShortName'] = 'Kidchess Office';
    $row['sSt:FirstName'] = 'Kidchess Office';
    $row['sSt:LastName'] = '';
...
gwy_fetch_selectMap_myStaff($appGlobals);
